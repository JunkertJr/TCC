...
//	private JTextField fieldNome = new JTextField(15);
	...
	public TelaManterCor() {		
		...
		fieldCodigo.addFocusListener(new FocusListener() {			
			@Override
			public void focusLost(FocusEvent e) {
				if (fieldCodigo.getText().trim().length() == 0) {
					JOptionPane.showMessageDialog(null, "FORMULÁRIO: Digite um código válido!");
				}
			}
			
			@Override
			public void focusGained(FocusEvent e) {
				//não faremos nada aqui pois não nos importa tratar o momento em que o cursor entrou no campo...
			}
		});
